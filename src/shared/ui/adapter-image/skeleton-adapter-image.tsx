...
export const SkeletonAdapterImage: FC<ISkeletonAdapterImageProps> = ({ status, className }) => (
  ...
  >
    { status === STATUS_LOADED_IMAGE.LOADING
      ? (
        <CameraIcon className="w-10 h-10 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 fill-slate-400" />
      )
      : (
        <ExclamationCircleIcon className="w-10 h-10 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 fill-slate-400" />
      ) }
  </div>
);
